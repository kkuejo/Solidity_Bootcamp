...
- 旧PoW時代のマイニング難易度を設定するパラメータ
- PoS移行後は使用されないが、genesis.jsonには設定が必要
- プライベートネットでは `"1"` または `"0x1"` を設定
- 実際のブロック生成は、Prysmコンセンサスクライアントが担当
...
**PoS対応の重要なパラメータ**:
- `mergeNetsplitBlock`: The Mergeの適用ブロック（0に設定することで最初からPoS）
- `terminalTotalDifficulty`: PoWからPoSへの移行閾値（0でPoS開始）
- `terminalTotalDifficultyPassed`: PoSに移行済みであることを示すフラグ
- `shanghaiTime`: Shanghai アップグレード（ステーキング引き出し機能）の適用時刻
...
executionディレクトリにいることを確認してください（ステップ2で作成済み）。
...
│       │   └── nodekey        # ノードの秘密鍵
│       └── keystore/          # アカウントの秘密鍵
└── consensus/             # コンセンサスレイヤー(Prysm)用ディレクトリ(今後使用予定)
```

### 3.5 ステップ4: プライベートネットワークの起動


まず、executionディレクトリに移動します。

```bash
cd ~/eth-private-pos/execution
```
...
- まだ新しいブロックが生成されていない
- コンセンサスクライアント（Prysm）が未接続のため、ブロック生成が行われていない
...
- 次回学習予定のPoS実装では、バリデータとしてステーキングを行い、ブロック生成報酬を得る
...
```bash
~/eth-private-pos/execution/mychaindata/keystore/
```
...
**注**: The Merge以降、Gethは実行レイヤーとして機能し、Prysmなどのコンセンサスクライアントと連携してPoS（Proof of Stake）ネットワークを構成します。次回の学習では、Prysmとの連携によるPoSネットワークの構築を行います。
...
**注**: 本日の学習では、Gethのインストールとプライベートネットワークの基本構築のみを行いました。

**PoS（Proof of Stake）によるブロック生成について**:

The Merge（2022年9月）以降、Ethereumはマイニング（PoW）からステーキング（PoS）に移行しました。本番環境と同じPoS方式でブロックを生成するには、以下の2つのクライアントが必要です：

1. **実行レイヤー（Execution Layer）**: Geth
   - スマートコントラクトの実行
   - トランザクション処理
   - 状態管理

2. **コンセンサスレイヤー（Consensus Layer）**: Prysm
   - ブロックの提案と検証
   - バリデータの管理
   - ステーキング報酬の分配

**次回の学習予定**:
次回は、**Prysm**コンセンサスクライアントをインストールし、Gethと連携させてPoSベースのプライベートネットワークを構築します。バリデータを設定し、ステーキングによるブロック生成を実際に体験します。
...
| 起動速度 | 遅い | 非常に速い |
| ブロック生成 | Prysmとの連携が必要（PoS） | 自動（即座に生成） |
| アカウント | 自分で作成 | 10個自動生成 |
