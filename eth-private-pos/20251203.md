...
1. **Beacon Chain（ビーコンチェーン）**:
   ...
   - バリデータの状態を管理
   - ブロックの提案スケジュールを決定
   - **簡単に言うと**: 交通整理をする信号機のようなもの。「次は誰がブロックを作る番か」を管理する

2. **Validator（バリデータ）**:
   - 32 ETHをステーキング（本番環境。プライベートネットでは不要）
   - ブロックを提案・証明
   - 報酬を獲得
   - **簡単に言うと**: ブロックを作る人。正しく作業すれば報酬がもらえる。不正をするとペナルティを受ける
...
- **CONFIG_NAME**: ネットワークの名前。`mainnet`など既存の名前を使うと、Prysmが「あ、Ethereumメインネットだな」と勘違いして、2020年のgenesis timeを使ってしまう。だから`private-pos`という固有の名前をつける。

- **GENESIS_FORK_VERSION**: ネットワークの識別番号のようなもの。mainnetと同じ`0x00000000`だと混乱するので、`0x10000000`という別の番号を使う。

- **SECONDS_PER_SLOT**: ブロックが作られる間隔。12秒に1回、新しいブロックができる。

- **SLOTS_PER_EPOCH**: エポック（期間）の長さ。32スロット = 384秒（6分24秒）で1エポックになる。これはバリデータの役割分担を決める単位。
...
- **genesis.json（Geth用）**: 「このネットワークのID（chainId）は32382です」「ガスの上限はこれです」といった、実行レイヤーのルールブック

- **genesis.ssz（Prysm用）**: 「ネットワークの開始時刻はいつです」「バリデータは何人います」といった、コンセンサスレイヤーのルールブック

- 両方とも「ブロックチェーンの最初の状態」を定義するファイルだが、それぞれのレイヤーで必要な情報が違う
...
1. **Validator（料理長）**: 「今日の料理は私が作る番だ」
2. **Beacon Chain（マネージャー）**: 「Gethさん、材料を集めて料理を準備して」
3. **Geth（調理師）**: トランザクション（材料）を集めてブロック（料理）を作る → 「できました！」
4. **Beacon Chain（マネージャー）**: 料理をチェックして → 「料理長、これでOKです」
5. **Validator（料理長）**: 最終チェックして署名（サイン） → 「完成！お客さんに出します」
6. **完成**: ブロックチェーンに記録される（お客さんに提供される）
...
### 13.5 Beacon Chainの「Failed to find peers」警告

**症状**:
```
time="..." level=error msg="Failed to find peers" error="find peers with subnets: context deadline exceeded" prefix=p2p
time="..." level=warning msg="Sync Committee Message is too old to broadcast, discarding it" error="sync message is too late
```

**原因**:
- プライベートネットワークで単一ノード構成のため、ピアが存在しない
- `--bootstrap-node ""`を設定しているため、他のノードと接続しない
- `--nodiscover`（Geth側）により、P2Pピア検索が無効化されている

**これは正常な動作です**:

プライベートネットワーク（単一ノード構成）では、以下の警告は**無視して問題ありません**：

| 警告メッセージ | 説明 | 影響 |
|-------------|------|------|
| `Failed to find peers` | 他のノードが見つからない | なし（ブロック生成には影響しない） |
| `Sync Committee Message is too old` | 同期委員会メッセージの遅延 | なし（単一ノードでは同期委員会は不要） |

**確認方法**:

これらの警告が出ていても、以下が正常であれば問題ありません：
...
# Beacon Head Slotが増加しているか確認
curl -s http://localhost:3500/eth/v1/beacon/headers/head | python3 -c "import sys, json; data = json.load(sys.stdin).get('data', {}); header = data.get('header', {}).get('message', {}); print('Head Slot:', header.get('slot'))"
```

**ブロック番号とHead Slotが12秒ごとに増加していれば、ネットワークは正常に動作しています。**

**メモ**:
- 本番環境やテストネットでは、複数のノードがP2Pネットワークで接続されるため、この警告は出ません
- プライベートネットワークで複数ノードを構成する場合は、`--bootstrap-node`で他のノードを指定する必要があります
...
| Failed to find peers警告 | 問題なし（単一ノード構成では正常） | プライベートネットの警告を理解する |
...
1. **CONFIG_NAMEは必ず固有の名前**:
   ...
   - **なぜ?**: 既存のネットワーク名を使うと、Prysmが「あ、このネットワーク知ってる！」と勘違いして、そのネットワークの設定（genesis time等）を勝手に使ってしまう

2. **GENESIS_FORK_VERSIONも固有の値**:
   - ✅ `0x10000000`, `0xAABBCCDD`
   - ❌ `0x00000000`（mainnetと同じ）
   - **なぜ?**: ネットワークの識別番号。mainnetと同じ番号だと「あれ？これってmainnetじゃないの？」と混乱する

3. **deposit_data.jsonは使用しない**:
   - プライベートネットワークでは`--num-validators`のみ
   - Interopモードで十分
   - **簡単に言うと**: プライベートネットでは複雑な手続きは不要。`--num-validators 1`だけでバリデータが作られる

4. **データのクリーンアップは完全に**:
   - `rm -rf consensus/beacon-data`
   - 中途半端なクリーンアップは問題を引き起こす
   - **例え**: 料理をやり直すとき、古い材料が残っていると新しい料理がうまく作れない。全部捨ててから新しく作る

5. **ログを注意深く読む**:
   - エラーメッセージは問題解決の鍵
   - `grep -i error`で素早くエラーを発見
   - **コツ**: エラーメッセージは難しく見えるけど、よく読むと「何が問題か」「どこで起きたか」が書いてある。Google検索も活用しよう

## 16. 次のステップ

### 16.1 次のステップ：ネットワークの操作

構築したネットワークを実際に**使ってみましょう**！

1. **JSON-RPC APIの使用**（2025-12-08に実施）:
   - Gethへの接続方法（HTTP、IPC）
   - アカウントの作成と管理
   - ブロック情報の取得
   - 残高の確認

2. **Beacon Chain APIの使用**（2025-12-08に実施）:
   - バリデータ情報の確認
   - スロット/エポック情報の取得
   - ネットワークの同期状態確認
...
4. **スマートコントラクトのデプロイ**（今後の課題）:
   - Solidityでシンプルなコントラクトを作成
   - Remixやハードハットでコンパイル
   - プライベートネットワークにデプロイ
   - コントラクトとのやり取り

5. **Web3.jsやEthers.jsとの統合**（今後の課題）:
...
3. **2025-12-08**: JSON-RPC APIとBeacon Chain APIの実践（アカウント作成、ブロックチェーン情報の取得）
4. **次回**: トランザクション送信とスマートコントラクトのデプロイ
